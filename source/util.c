...
Result pauseInit()
{
    ...
    FILE* should_pause_file = fopen("/config/sys-httpd/httpd_paused", "r");
...
void setPaused(bool newPaused)
{
    ...
    if (paused)
    {
        FILE* should_pause_file = fopen("/config/sys-httpd/httpd_paused", "w");
        fclose(should_pause_file);
        flash_led_pause();
    }
    else
    {
        unlink("/config/sys-httpd/httpd_paused");
        flash_led_unpause();
    }
    mutexUnlock(&pausedMutex);
}
